...
const ExploreCard = ({ id, imgUrl, title, index, active, handleClick }) => (
  ...
  >
    <Image src={imgUrl} alt={title} className="absolute w-full h-full object-cover rounded-[24px]" />
    ...
    {active !== id ? (
      ...
      <div className="absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
        <div className={`${styles.flexCenter} w-[60px] h-[60px] glassmorphism mb-[16px] rounded-[12px]`}>
          <Image src="/headset.svg" alt="headset" className="w-1/2 h-1/2 object-cotain" />
        </div>
        <p className="font-normal text-[16px] leading-[20.16px] text-white uppercase">Mercado Financiero</p>
